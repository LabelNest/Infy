...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'refinery' | 'vault' | 'logistics'>('refinery');
  ...
  const refreshCredits = async () => {
    ...
    const bal = await getUserBalance(currentUser);
    setCredits(bal);
  };

  const handleLogout = () => {
    localStorage.removeItem('labelnest_identity');
    sessionStorage.removeItem('labelnest_admin_active');
  ...
  const loadVault = async () => {
    try {
      ...
      console.error("Infy Vault load failed:", e); 
    }
  };
  ...
  useEffect(() => {
    if (!isEngineActive || isProcessing || processingRef.current || credits <= 0 || !isAuthenticated) return;
    ...
    const processQueue = async () => {
      ...
      const updateLead = async (updates: Partial<LeadRecord>) => {
        ...
        if (updates.state === 'completed') {
          ...
          try {
            await saveEnrichedLead(finalRecord as LeadRecord, currentUser);
            await refreshCredits();
          } catch (e: any) {
            updateLead({ state: 'error', last_stage: 'ERROR', error: e.message || 'Vault Sync Failed' });
          }
        }
      };
    ...
    processQueue();
  }, [leads, isEngineActive, isProcessing, credits, isAuthenticated]);
  ...
  const handleAddLead = (input: LeadInput) => {
    if (credits <= 0) {
      alert("Insufficient credits. Pipeline halted.");
      return;
    }
    setLeads(prev => [{
  ...
  if (!isAuthenticated) {
    return <AuthGuard onAuthenticated={(email) => {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <div className={`w-2 h-2 rounded-full ${isEngineActive && credits > 0 ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500 animate-bounce'}`}></div>
                <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">
                  {credits > 0 ? `Intelligence Refinery Active` : 'Pipeline Halted: No Credits'}
                </span>
              </div>
            </div>
          </div>
          <nav className="flex bg-white/5 p-1.5 rounded-xl border border-white/5">
            <button onClick={() => setActiveTab('refinery')} className={`px-6 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'refinery' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>Refinery</button>
            <button onClick={() => setActiveTab('vault')} className={`px-6 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'vault' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>Vault</button>
            <button onClick={() => setActiveTab('logistics')} className={`px-6 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'logistics' ? 'bg-amber-500 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>Logistics</button>
          </nav>
        </div>

        <div className="flex items-center gap-6">
          <div className="flex flex-col items-end">
            <div className="flex bg-indigo-900/40 px-4 py-2 rounded-full border border-indigo-500/30 items-center gap-3">
              <span className="text-[10px] font-black text-indigo-300 uppercase tracking-widest">Balance</span>
              <span className="text-white font-black text-xs">⚡ {credits}</span>
            </div>
            <div className="flex items-center gap-2 mt-2">
               <span className="text-[8px] font-black uppercase text-slate-500">{currentUser}</span>
               {currentUser === 'ankit@labelnest.in' && <span className="text-[7px] bg-indigo-600 px-1.5 py-0.5 rounded text-white font-black uppercase tracking-widest">Owner</span>}
               <button onClick={handleLogout} className="text-[8px] font-black uppercase text-rose-400 hover:text-rose-500 ml-2 cursor-pointer transition-colors">Logout</button>
            </div>
          </div>
        ...
        {activeTab === 'refinery' && (
          <div className="grid grid-cols-12 gap-10">
            <aside className="col-span-12 lg:col-span-4 space-y-10">
              ...
              <BulkEngine onBulkAdd={(inputs) => {
                if (credits < inputs.length) {
                  alert(`Batch exceeds balance. You have ${credits} credits, but tried to process ${inputs.length}.`);
                  return;
                }
                setLeads(prev => [...inputs.map(i => ({ id: crypto.randomUUID(), batchId: 'BULK', input: i, state: 'queued' as any, progress: 0, last_stage: 'QUEUED' as any, enriched: null, createdAt: Date.now() })), ...prev])
              }} isProcessing={isProcessing} leads={leads} />
              <StateMappingTool />
            </aside>
